...
const EditVisaRequirement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const requirementRes = await fetch(`/api/internal/visa-requirements/${requirementId}`);
        if (!requirementRes.ok) throw new Error('Failed to fetch requirement');
        
        const requirementData = await requirementRes.json();
        if (!requirementData?.data) throw new Error('Requirement data not found');
        
        setForm({
          visa_country_id: countryId,
          visa_type_id: requirementData.data.visa_type_id.toString(),
          title: requirementData.data.title,
          description: requirementData.data.description,
          visa_country_name: requirementData.data.visa_country_name,
          visa_type_name: requirementData.data.visa_type_name
        });
    ...
    if (countryId && requirementId) {
      fetchData();
    }
  }, [countryId, requirementId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/internal/visa-requirements/${requirementId}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to update visa requirement');
      }

      alert('Visa Requirement Updated Successfully!');
      router.push(`/admin/visit-visa/details/${countryId}`);
    } catch (err) {
      console.error('Error:', err);
      setError(err.message || 'Failed to update visa requirement');
    } finally {
    ...
    <Layout>
      <div className="p-6 max-w-2xl mx-auto bg-white rounded-lg shadow-md">
        <h1 className="text-2xl font-bold mb-6 text-gray-800">
          Edit Visa Requirement for {country.country_name}
        </h1>
...
export default EditVisaRequirement; 